...
class MainWindow(QMainWindow):
    ...
    def add_checkboxes_to_page_2(self):
        ...
        try:
            ...
            for serial_id, name, genre, country, year, seasons, episodes, pic in serials:
                ...
                serial_widget.setContentsMargins(20, 20, 20, 20)
                serial_widget.setStyleSheet("border: 2px solid #ffffff; font-size:20px")

                image_label = QLabel()
                if pic is not None:
                    image_data = base64.b64decode(pic)
                else:
                    with open('zat3.png', 'rb') as f:
                        image_data = f.read()
                pixmap = QPixmap()
                pixmap.loadFromData(image_data)
                image_label.setPixmap(pixmap.scaled(300, 450, aspectRatioMode=Qt.AspectRatioMode.KeepAspectRatio))
    ...
    def add_checkboxes_to_page_3(self):  #watching
        ...
        try:
            ...
            for serial_id, name, genre, country, year, seasons, episodes, pic in serials:
                ...
                end_watching = QPushButton("Закончить просмотр")
                end_watching.clicked.connect(lambda state, serial_id1=serial_id: self.end_watching(serial_id1))
                checkbox_layout.addWidget(end_watching)
    ...
    def add_checkboxes_to_page_4(self): #planned
        ...
        try:
            ...
            for serial_id, name, genre, country, year, seasons, episodes, pic, date in serials:
                ...
                serial_layout = QHBoxLayout(serial_widget)
                serial_widget.setContentsMargins(20, 20, 20, 20)
                serial_widget.setStyleSheet("border: 2px solid #ffffff; font-size:20px; border-radius:10px;")
                ...
                start_watching.clicked.connect(lambda state, serial_id1=serial_id: self.start(serial_id1))
    ...
    def end_watching(self, serialid):
        self.remove_from_table('watching', serialid)
        self.add_to_table('watched', serialid)
        self.add_checkboxes_to_page_3()
        self.add_checkboxes_to_page_5()
        self.add_checkboxes_to_page_2()
    ...
    def filec(self):
        ...
        document.add_heading('Смотрю')
        cursor.execute(
            "SELECT serials.id, serials.name, serials.genre, serials.country, serials.year, serials.seasons, serials.episodes, serials.pic FROM serials INNER JOIN watching ON serials.id = watching.idSerial")
        serials = cursor.fetchall()

        for serial_id, name, genre, country, year, seasons, episodes, pic in serials:
            result = cursor.execute("SELECT name FROM genres WHERE id = ?",
                                    (genre,)).fetchall()
            text_to_save = f"{name}: {result[0][0]}, {country}, {year}, {seasons}, {episodes}\n"
            document.add_paragraph(text_to_save)
        document.add_heading('Просмотрено')
        cursor.execute(
            "SELECT serials.id, serials.name, serials.genre, serials.country, serials.year, serials.seasons, serials.episodes, serials.pic FROM serials INNER JOIN watched ON serials.id = watched.idSerial")
        serials = cursor.fetchall()
    ...
    def delete_serial(self):
        try:
            ...
            serial_id = cursor.execute("SELECT id FROM serials WHERE name = ?", (name,))
            self.remove_from_table('planned', serial_id)
            self.remove_from_table('watching', serial_id)
            self.remove_from_table('watched', serial_id)
